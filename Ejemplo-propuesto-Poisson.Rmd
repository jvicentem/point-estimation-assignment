...
Histograma de la función a priori:
...
poisson_gamma_values <- data.frame(MCpoissongamma(gamma_values$values, alpha = 1, beta = 2, mc = 100))
colnames(poisson_gamma_values) <- c('values')

poisson_gamma_values %>%
  ggplot(aes(x=values)) +
  geom_histogram(aes(fill=..count..), binwidth = 0.01, col='black')

poisson_gamma_values %>%
  ggplot(aes(x=values)) +
  geom_density(alpha=.2, fill="#FF6666")

summary(poisson_gamma_values)
```

Un estimador puntual de $\lambda$ podría ser la media, que en este caso es:
```{r bayes-c} 
mean(poisson_gamma_values$values)
```

Antes hemos utilizado la librería MCMCpack que trabaja con métodos Markov chain Monte Carlo. Ahora vamos a realizar una estimación puntual de $\lambda$ sin ayuda de ninguna librería que calcule cómo sería la función a posteriori.
...
post_gamma_values %>%
  ggplot(aes(x=lambda_seq, y=prior_gamma_values$values)) +
  geom_line()+
  ylim(0, 0.4)
...
post_gamma_values %>%
  ggplot(aes(x=lambda_seq, y=post_gamma_values$values)) +
  geom_line() +
  ylim(0, 4)
